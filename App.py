...
def get_movie_info(imdb_link):
    ...
    movie_cast = str(movie_descr[1]).replace('Stars', 'Cast: ').strip()
    movie_story = 'Story: ' + str(movie_descr[2]).strip() + '.'
    award = s_data.find("span", class_="ipc-metadata-list-item__list-content-item").text
    film_acv = 'FILM AWARDS: ' + str(award)
    return  movie_story, film_acv
...
def run():
    ...
    if cat_op == category[0]:
        ...
        st.markdown(
            '''<h4 style='text-align: left; color: #d73b5c;'>WAIT FOR RESPONSE</h4>''',
            unsafe_allow_html=True)
        if dec == 'No':
            if select_movie == '--Select--':
                ...
                for movie, link, ratings in table:
                    c += 1
                    story, total_rat = get_movie_info(link)
